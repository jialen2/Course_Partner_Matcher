...
class SignUpViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request):
        ...
        try:
            user = User.objects.raw('SELECT * FROM LoginInfo where AccountName = %s', [username])
            ...
            instance = User.objects.raw('Update LoginInfo SET AccountName = %s, Password=%s, [AccountName, Password]')
            #instance = User(username=username, password=password, email=email, firstname=firstname, lastname=lastname)
            instance.save()
            return Response({"response": {"error":"OK", "id": instance.id, "AccountName": instance.AccountName, "Password":instance.Password}, "status": 201}, status=status.HTTP_201_CREATED)
        else:
